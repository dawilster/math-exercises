...
def pretty_module_name(dirname: str) -> str:
    if dirname == "interludes":
        return "🌀 Wonder Interludes"
    num, _, rest = dirname.partition("-")
    words = rest.replace("-", " ").title()
    words = NAME_FIXES.get(words, words)
    icon = MODULE_ICONS.get(num, "📐")
    return f"{icon} Module {int(num)} — {words}"
...
def scan_modules() -> list[dict]:
    ...
    for mdir in sorted((ROOT / "modules").iterdir()):
        ...
        modules.append({"dir": mdir.name, "name": pretty_module_name(mdir.name),
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .module h2 { font-size:1.05rem; margin:.2rem 0 .7rem; display:flex; align-items:center; gap:.6rem; }
  ...
  .tick { width:1.45rem; height:1.45rem; border:2px solid #cbd5e1; border-radius:7px; cursor:pointer;
          flex:none; display:flex; align-items:center; justify-content:center; font-size:.95rem;
...
<div id="modules"></div>
<footer>Tick a unit when its worksheet is done &amp; scanned · state saved to <code>profile/dashboard-state.json</code></footer>
...
function render() {
  ...
  if (inbox > 0) {
    ...
    el.style.display = "";
    el.textContent = "📸 " + inbox + " scan" + (inbox > 1 ? "s" : "") + " waiting in inbox — tell Claude!";
  }
  let nextFound = false;
  ...
  for (const m of modules) {
    ...
    for (const u of m.units) {
      ...
      row.innerHTML = `
        <div class="tick" title="mark complete">✓</div>
        <span class="title">${u.title}${isNext ? ' <span class="nextlabel">← NEXT UP</span>' : ""}</span>
        ${isDone ? `<span class="when">${completed[u.id]}</span>` : ""}
        <span class="links">
          ${u.lesson ? `<a href="/view/${u.lesson}">📖 lesson</a>` : ""}
          ${u.worksheet ? `<a href="/view/${u.worksheet}">✍️ worksheet</a>` : ""}
          ${u.notebook ? `<button data-nb="${u.notebook}">💻 notebook</button>` : ""}
        </span>`;
      row.querySelector(".tick").onclick = async () => {
        const r = await fetch("/api/toggle", { method: "POST", body: JSON.stringify({ id: u.id }) });
...
def nav_bar(rel_path: str) -> str:
    seq = study_sequence()
    idx = next((i for i, s in enumerate(seq) if s["path"] == rel_path), None)
    prev_link = next_link = ""
    if idx is not None and idx > 0:
        p = seq[idx - 1]
        prev_link = f'<a href="/view/{p["path"]}">← {p["kind"]}: {p["title"]}</a>'
...
def dashboard_html() -> str:
    data = {"modules": scan_modules(), "state": load_state(),
